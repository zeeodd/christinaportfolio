...
function showInfo(data) {
  ...
  for (var i = 0; i < WEEKSCURRENT1; i++) {
      if (data[i]["MATH260FA_HW_Name"] == "" || data[i]["MATH260FA_HW_PDF"] == "") {
        ...
        $("#courseCurrent1" + "-W" + String(i+1)).html("Week " + String(i+1) + ": " + '<a href=' + String(data[i]["MATH260FA_HW_PDF"]) + ' target="_blank">' + String(data[i]["MATH260FA_HW_Name"]) + '</a>');
      }
    }
  ...
  for (var i = 0; i < WEEKSCURRENT2; i++) {
      if (data[i]["MATH150FA_HW_Name"] == "" || data[i]["MATH150FA_HW_PDF"] == "") {
        ...
        if (debug) console.log(data[i]["MATH150FA_HW_PDF"]);
        $("#courseCurrent2" + "-W" + String(i+1)).html("Week " + String(i+1) + ": " + '<a href=' + String(data[i]["MATH150FA_HW_PDF"]) + ' target="_blank">' + String(data[i]["MATH150FA_HW_Name"]) + '</a>');
      }
    }
